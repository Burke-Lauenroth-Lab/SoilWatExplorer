...
MouseWheelListener {
    ...
    public void mouseClicked(MouseEvent e) {
    	if(e.getClickCount() == 1 && e.getButton() == MouseEvent.BUTTON1 && map.getZoom() >= 6){
    		Point p = e.getPoint();
    		int X = p.x+3;
    		int Y = p.y+3;
    		...
    		while (i.hasNext()) {
    			...
    			if( MarkerPosition != null){
    				...
    				int site_id = ((MapMarkerDot) mapMarker).getID();
    				...
    				if (radCircle < 8){
    					...
    					if(site_id > 0) {
    						int last_id = -1;
    						if(map.folder.getText() == "")
    							last_id = Integer.valueOf(map.folder.getText().replaceAll("Site: ", ""));
    						if(site_id != last_id) {
    							map.siteEvent.siteSelected(site_id);
    							map.folder.setText("Site: "+String.valueOf(site_id));
    						}
    					} else
    						map.folder.setText("Weather_NCEPCFSR_"+String.format("%.3f", mapMarker.getLat())+"-"+String.format("%.3f", mapMarker.getLon()));
    					break;
    				}
    			}
    		}
